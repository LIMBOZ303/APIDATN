...
const populatePlans = async (plans) => {
    return await Promise.all(plans.map(async (plan) => {
        // Thêm xử lý lockedData
        if (plan.status === 'Đã đặt cọc' && plan.lockedData) {
            return {
                ...plan.toObject(),
                SanhId: plan.lockedData.sanh,
                caterings: plan.lockedData.caterings,
                decorates: plan.lockedData.decorates,
                presents: plan.lockedData.presents,
                totalPrice: plan.lockedData.totalPrice,
                plansoluongkhach: plan.lockedData.plansoluongkhach,
                plandateevent: plan.lockedData.plandateevent,
                name: plan.lockedData.name
            };
        }
    ...
    try {
        ...
        if (oldPlan.status === 'Đã đặt cọc') {
            return res.status(403).json({
                status: false,
                message: "Kế hoạch đã đặt cọc và không thể cập nhật."
            });
        }
...
router.put('/override/:planId', async (req, res) => {
    try {
        ...
        if (originalPlan.status === 'Đã đặt cọc') {
            return res.status(403).json({
                success: false,
                message: "Kế hoạch đã đặt cọc và không thể ghi đè."
            });
        }
...
router.post('/confirm/:tempPlanId', async (req, res) => {
    try {
        ...
        if (originalPlan.status === 'Đã đặt cọc') {
            return res.status(403).json({
                success: false,
                message: "Kế hoạch gốc đã đặt cọc và không thể xác nhận thay đổi."
            });
        }
...
// Chuyển trạng thái từ "Đang chờ xác nhận" sang "Chưa đặt cọc"
...
router.put('/cancel/:planId', async (req, res) => {
    try {
        ...
        // Thêm xử lý lockedData
        if (plan.status === 'Đã đặt cọc') {
            return res.status(403).json({
                status: false,
                message: "Kế hoạch đã đặt cọc và không thể hủy."
            });
        }
...
router.delete('/:planId', async (req, res) => {
    try {
        ...
        if (plan.status === 'Đã đặt cọc') {
            return res.status(403).json({
                status: false,
                message: "Kế hoạch đã đặt cọc và không thể xóa hoặc chỉnh sửa dịch vụ."
            });
        }
...
router.delete('/user/:userId/plan/:planId', async (req, res) => {
    try {
        ...
        if (plan.status === 'Đã đặt cọc') {
            return res.status(403).json({
                status: false,
                message: "Kế hoạch đã đặt cọc và không thể xóa."
            });
        }

        await Plan.findOneAndDelete({ _id: planId, UserId: userId });
        await Promise.all([
            Plan_catering.deleteMany({ PlanId: planId }),
            Plan_decorate.deleteMany({ PlanId: planId }),
            Plan_present.deleteMany({ PlanId: planId })
